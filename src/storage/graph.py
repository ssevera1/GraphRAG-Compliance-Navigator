...
class KnowledgeGraph:
    ...
    def add_relationship(self, rel: Relationship) -> None:
        ...
        if not rel.source or not rel.source.strip():
            logger.warning("Skipping relationship with empty source")
            return
        if not rel.target or not rel.target.strip():
            logger.warning("Skipping relationship with empty target")
            return
        query = (
        ...
        try:
            with self._driver.session() as session:
                ...
                if summary.counters.relationships_created == 0:
                    logger.warning(
                        f"Relationship {rel.source}-[{rel.type.value}]->"
                        f"{rel.target} matched existing edge"
                    )
                else:
                    logger.debug(
                        f"Created relationship: {rel.source}-[{rel.type.value}]->"
                        f"{rel.target}"
                    )
        except DriverError as e:
            logger.error(
    ...
    def add_extraction(self, result: ExtractionResult) -> None:
        ...
        if not result.entities and not result.relationships:
            ...
            return
        logger.info(
            f"Adding extraction with {len(result.entities)} entities and "
    ...
    def get_neighbours(self, entity_name: str) -> list[dict]:
        ...
        if not entity_name or not entity_name.strip():
            logger.warning("Empty entity_name provided to get_neighbours")
            return []
        query = (
